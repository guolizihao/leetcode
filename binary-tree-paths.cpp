...
public:
    vector<string> binaryTreePaths(TreeNode* root) {
        ...
        if (!root) return res;
        string s;
        binaryTreePathsDFS(root, s, res);
        return res;
    }
    
    void binaryTreePathsDFS(TreeNode* root, string s, vector<string> &res) {
        s += itoa(root->val);
        ...
        s += "->";
        if (root->left) {
            binaryTreePathsDFS(root->left, s, res);
        }
        if (root->right) {
            binaryTreePathsDFS(root->right, s, res);
        }
    }
